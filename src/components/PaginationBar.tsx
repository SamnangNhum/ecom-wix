    ...
    return <Pagination>
        <PaginationContent>
            ...
            {Array.from({ length: totalPages }).map((_, i) => {
                ...
                return <PaginationItem key={page} className={cn("hidden md:block", page === currentPage && "pointer-events-none block")}>
                    <PaginationLink href={getLink(page)} isActive={page === currentPage}>
                        {page}
                    </PaginationLink>
                </PaginationItem>
            })}
            <PaginationItem>
                <PaginationNext
                    href={getLink(currentPage + 1)}
                    className={cn(
                        currentPage >= totalPages && "text-muted-foreground pointer-events-none"
                    )}
                />
            </PaginationItem>
        </PaginationContent>
